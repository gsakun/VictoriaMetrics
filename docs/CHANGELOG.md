...
* FEATURE: [vmagent](https://docs.victoriametrics.com/vmagent.html): add `dryRun.syntaxOnly` and `promscrape.config.dryRun.syntaxOnly` flags, will only check config syntax during dryrun, ignore file references in auth configs. See [this issue](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/4625) for details.
